...
**E3**（4 后端 × 5 配对 × 3 场景 × 5 重复 = **60 场锦标赛、600 个臂**，每个臂都强制派工）是第一轮拿到正向结果的实验，而且它复制成功：历史接线在 doubao、glm、minimax、qwen 上分别赢下 **25/26、28/32、16/20、19/22** 组 resolved 配对。20 个「后端×政体」组合与 12 个「后端×场景」组合的均值 Δ 全为正。E1 的长度混淆是被**测量**排除的，而不是被论证排除的——源臂的原始 transcript 比对照臂长 1.4–2.5 倍，但**两侧各约 100% 的臂都触及 6,000 字符预算上限**，裁判从两边读到的量是一样的。

有三条限定与这个结果同行，它们不是脚注：

1. **D2 的阈值写法有歧义，而选哪种读法会改变结论。** 采用更严的「按后端汇总噪声地板」读法时，minimax 的效应完全消失（0 源 / 1 对照），qwen 只剩两组配对支撑。「四中之四」属于按场读法；保守读法给出的是四中之三。
2. **拓扑与文本质量没有被分离。** 对照臂重写了决策流程，但一段被重连的流程同时也是一段**机器写的**流程。裁判有可能是在奖励原始文档的连贯度，而非它编码的结构。E3 里没有任何设计能区分这两者——下一个实验应当是「保留人写散文、只重连拓扑」的对照。
3. **这个效应依赖强制。** E3 每个臂都带 `--enforce-dispatch`；不带强制的 E1-control 里，源臂赢下 0 组。而 D4 显示这套接线**并不会被自发采纳**：没有任何模型凭空发明拓扑未声明的职官，但**每个模型都会漏掉已声明的职官**——平均每份计划漏 0.84 到 2.11 个——完整复现声明名单的计划只占 13%（doubao）到 57%（qwen）。

所以诚实的现状是：**在被强制执行的前提下，历史接线在四个独立模型族上都胜过打乱的接线——而模型基本不会自己强制自己。** 这比 §1.2 的「可复用编排模式」要窄：可复用同时要求「有效」与「被采纳」，目前只有前者。完整结果见 `docs/experiments/E3-results.md`。
