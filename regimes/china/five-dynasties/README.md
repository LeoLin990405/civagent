...
|---|---|---|---|
| 朝廷 (Court) | 名义中央朝廷 | 发布任务目标、名义协调、汇总结果、选择最优方案 | Tier-1 (Coordinator): GPT-4o / Claude |
| 节度使A (Warlord A) | 独立藩镇（如河东节度使） | 方案A生成、独立代码实现、专精领域A（如前端） | Tier-2 (Specialist): Gemini / Qwen |
| 节度使B (Warlord B) | 独立藩镇（如淮南节度使） | 方案B生成、独立代码实现、专精领域B（如后端） | Tier-2 (Specialist): DeepSeek / Kimi |
| 节度使C (Warlord C) | 独立藩镇（如西川节度使） | 方案C生成、独立代码实现、专精领域C（如算法） | Tier-2 (Specialist): Codex o3 / DeepSeek |
| 掌书记 (Scribe) | 幕府文书、跨镇联络 | 信息汇总、方案对比、冲突调停、结果记录 | Tier-3 (Support): GPT-4o-mini / Kimi |
...
- **多模型基准测试 / Multi-Model Benchmarking**: 让不同 AI 模型（GPT-4o、Claude、Gemini）各自执行同一任务，对比输出质量，选择最优。
